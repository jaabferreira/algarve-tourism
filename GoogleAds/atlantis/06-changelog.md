...
## 2026-07-01 — ES/FR landing pages fully localized (FareHarbor SmartAds fix)

**What:** FareHarbor's Nicolás del Castillo reported (2026-06-30) that the French and Spanish site versions still showed English content in the user journey, hurting SmartAds landing-page quality. Root cause was a two-gate English fallback: `scripts/fetch-fh.ts` fetched tour data in PT+EN only, and `packages/atlantis/src/data.ts` mapped only en/pt at render time. FareHarbor already holds genuine es/fr product content (verified via the External API), so the fix was to open both gates (`FH_LANGUAGES` + `FH_LOCALE_MAP` now include es/fr) and re-fetch. Tour slugs are derived per-language, so es/fr tour URLs moved from English-fallback slugs to native slugs (e.g. `/es/tours/circuito-de-cuevas-de-benagil/`); added 8 × 301 back-redirects for the old URLs. Also threaded the page locale into the FareHarbor booking lightframe (`?language=`) so the booking modal opens in ES/FR. Two independent adversarial sweeps of the built pages then caught data/logic-driven English that static i18n missed and it was all fixed: category badges rendered the raw key "boats" (now localized), `parseDescription` leaked untranslated FareHarbor field keys (`max_age`, `group_size`, `highlights`, `check_in_details`, `special_requirements`) into the visible tour body (parser now treats all FH structural keys as boundaries), breadcrumb "Home" in JSON-LD, chrome/a11y aria-labels, image alt "photo" suffix, and review-card platform/tour labels. Algarve & You inherits the shared-component fixes but its tour content stays EN-fallback (no live ads; a trivial follow-up once its render map is wired the same way).

**Why:** English on the FR/ES ad destinations lowers Google's ad-relevance and landing-page-quality scores and wastes SmartAds budget (spending since ~2026-06-15). The visible content a French/Spanish visitor reads is now fully in-language.

**Expected effect:** higher landing-page-quality / ad-relevance on the FR + ES SmartAds; lower wasted spend and better CPC/quality on those locales. No campaign settings changed — landing-page content only. Deploys via CF Pages on push to `master` (CF re-runs the FH fetch, now incl. es/fr). **FareHarbor must repoint the FR/ES ad final URLs to the new native slugs** (the old English-slug URLs 301-redirect, but ad destinations should not rely on a redirect hop).

**Open item (content decision, not a bug):** the customer review testimonials on ES/FR pages remain in the reviewer's original (mostly English) language — authentic UGC that should not be machine-translated. Options for José: curate locale-specific reviews, or show a "translated" variant. All developer/platform-authored copy is localized.

**Verify on/after:** 2026-07-08 — Google Ads landing-page-experience / ad-relevance for FR+ES ad groups holds or improves; spot-check `/fr/` and `/es/` tour pages render fully in-language; confirm FH updated the ad final URLs.

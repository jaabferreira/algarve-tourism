...
- **Site-side wiring shipped** — `docs/superpowers/plans/2026-05-12-atlantis-content-hub-wiring.md` (branch `feat/atlantis-content-hub`, Tasks 1–9): hub-aware breadcrumbs, "Part of our complete guide" callout on cluster posts, "In this guide" cluster list on the pillar, `FAQPage` JSON-LD + visible FAQ block when a post has `faqs:`, a "Plan your trip" tour→guide block, the pillar pinned on the blog index + homepage, and `pillarSlug`/`pillarOrder` on the 8 existing cluster posts (locale-aware). No URL changes. Everything degrades gracefully on posts without the new frontmatter. Remaining work is the **content workstream** (below) + a follow-up to key the tour→guide map by `translationKey` so pt/es/fr tour pages also get the "Plan your trip" block (currently EN-only).
...
- ✅ `pillar-content-architecture` + ✅ site-side wiring — see `content-hub/2026-05-12-atlantis-benagil-hub-architecture.md` and `docs/superpowers/plans/2026-05-12-atlantis-content-hub-wiring.md` (both branch `feat/atlantis-content-hub`; structure/schema/components/breadcrumbs/tour-links all shipped, no URL changes). **Remaining = the content workstream:** per-piece briefs via `content-brief-authoring` then Opus drafts (order in §8 of the architecture doc) — the pillar rewrite (~1,060→~3,500w), the 2–3 new posts (`how-to-visit-benagil-cave`, `can-you-swim-benagil-cave`, `benagil-cave-tour-with-kids`) incl. their pt/es/fr mirrors, the de-dup edits across the existing posts, the in-body cross-links per `content-hub/2026-05-12-atlantis-benagil-hub-links.csv`, and `faqs:` frontmatter on the pillar + the Q&A clusters. New posts need `pillarSlug` (locale-specific) + `pillarOrder` when created. Plus the deferred `/benagil-cave-guide/` URL migration (architecture doc §3) and the `translationKey`-keyed tour→guide map follow-up.

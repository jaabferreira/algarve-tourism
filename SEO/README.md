...
- ✅ `pillar-content-architecture` + ✅ site-side wiring — design in `content-hub/2026-05-12-atlantis-benagil-hub-architecture.md`, wiring shipped on PR #2 (`feat/atlantis-content-hub`). **→ Remaining work is tracked in `SEO/content-hub/BUILD-STATUS.md`** (the content workstream — pillar rewrite, the 2–3 new posts + pt/es/fr mirrors, de-dup edits, in-body cross-links, `faqs:` content — plus the 3 minor site-wiring follow-ups and the deferred `/benagil-cave-guide/` URL migration).

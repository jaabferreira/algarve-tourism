...
2. ✅ **DONE 2026-05-12 (in repo, not yet deployed)** — **tour meta descriptions fixed**: new `truncateAtWord` helper (`packages/shared/src/lib/text.ts`) kills mid-word cuts for every tour×locale; new `packages/atlantis/src/lib/seo-overrides.ts` holds hand-written EN + PT copy + `<title>` overrides for the 4 tours; one value now feeds `<meta>`/OG/Twitter/`Product.description`. **TODO:** native ES + FR copy in `seo-overrides.ts` (ES/FR currently fall back to the truncated English FareHarbor description — no longer broken, but not localized). *(audit P0 #2)*
